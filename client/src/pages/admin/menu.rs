...
use mogwai::prelude::*;
use stream_log_shared::messages::admin::{AdminAction, MenuInfo};
...
pub async fn run_menu(
	...
	loop {
		let message = SubPageControl::Event(AdminAction::MenuInfo);
		let message_json = serde_json::to_string(&message)?;
		ws_write.send(Message::Text(message_json)).await?;
		let menu_data: DataMessage<MenuInfo> = read_websocket(ws_read).await?;
		let menu_data = menu_data?;
			...
			<div id="admin-menu">
				...
				<ul id="admin-menu-links">
					<li>
						<a
							class="click"
							on:click=click_tx.sink().contra_map(|_| AdminMenuItem::UnapprovedAccounts)
						>
							"Account Approval ("
							{menu_data.unapproved_user_count.to_string()}
							")"
						</a>
					</li>
					<li>
		...
		match clicked_option {
			AdminMenuItem::UnapprovedAccounts => todo!(),
